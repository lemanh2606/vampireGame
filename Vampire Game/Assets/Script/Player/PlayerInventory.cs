    ...
    public class Slot
    {
        ...
        public void Assign(Item assignedItem)
        {
            ...
            if (item is Weapon)
            {
                ...
                image.enabled = true;
                image.sprite = w.data.icon; // Assuming 'icon' is the correct property name
            }
            else
            {
                Passive p = item as Passive;
                image.enabled = true;
                image.sprite = p.data.icon; // Assuming 'icon' is the correct property name
            }
...
public void LevelUpPassiveItem(int slotIndex, int upgradeIndex)
    ...
    void ApplyUpgradeOptions()
    {
        ...
        foreach (UpgradeUI upgradeOption in upgradeUIOptions)
        {
            ...
            {
                // NOTE: We have to recede this system, as right now it disables an upgrade slot if
                ...
                if (chosenPassiveUpgrade != null)
                {
                    ...
                    // Loops through all our existing passives. If we find a match, we will
                    // Hook an event listener to the button that will level up the passive
                    // when this upgrade option is clicked.
                    bool isLevelUp = false;
                    for (int i = 0; i < passiveSlots.Count; i++)
                    {
